...
```
http://www.airbnb.com/s/Lucca--Italy?page=<page_number>
```
...
def parse(self, response):
    ...
    last_page = self.last_pagenumer_in_search(response)
    if last_page < 1:
        # return None if there is nothing to scrape
        return
    else:
        # create a list of urls which have the right structure
        page_urls = [response.url  + "?page=" + str(pageNumber)
                 for pageNumber in range(1, last_page_number + 1)]
        for page_url in page_urls:
            # sequentially request all the pages and pass the result to
            # parse_listing_results_page
